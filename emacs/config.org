...
#+END_SRC
  ...
  It's frequent enough that I have to do something for an operating system that
  I find it worth having specific functions for it. /However/, Emacs has some
  strange names for them (Windows is =windows-nt= for some reason). So I make
  some nice wrappers for them.
#+BEGIN_SRC emacs-lisp
...
#+END_SRC
   ...
   Replace the default window title with the name of the current project.
#+BEGIN_SRC emacs-lisp
...
#+BEGIN_SRC emacs-lisp
  (setq ejh/default-font "NotoSansMono")
  (setq ejh/default-font-size 12)
  ...
  (defun ejh/set-font-size ()
    "Set the font to `ejh/default-font' at at `ejh/current-font-size'.
...
* Package configuration
  ...
  and get it, but rather than typing =:ensure t= all over the place...
#+BEGIN_SRC emacs-lisp
...
#+BEGIN_SRC emacs-lisp
  ...
  ;; This requires some explaining. On a normal keyboard, this would be instanity
...
#+BEGIN_SRC emacs-lisp
  ...
  (general-define-key
   ...
   :states '(emacs visual visual-line)
...
*** Fill column
    ...
    with just regular 'IL =whitespace-mode= or something.
#+BEGIN_SRC emacs-lisp
...
#+BEGIN_SRC emacs-lisp
  ...
  (use-package git-gutter) ; TODO: Enable global git gutter mode?
#+END_SRC
*** Ivy / Counsel / Swiper
...
#+BEGIN_SRC emacs-lisp
  (use-package stupid-indent-mode
    :defer t) ; usually I'd avoid this, but I don't know what else to defer it on
#+END_SRC
*** Yasnippets
...
#+BEGIN_SRC emacs-lisp
  (use-package elisp-mode ; I think this is the right name to use
    ...
    (:prefix ejh/abnormal-leader-key
     :states '(visual visual-line emacs)
     "e" '(nil :which-key "evaluate")
     "eb" '(eval-buffer :which-key "buffer")
...
#+BEGIN_SRC emacs-lisp
  (use-package nim-mode
    :init
    (nim-mode-hook #'stupid-indent-mode) ; Hopefully that hook is there..
#+END_SRC
*** Nix
*** OCaml
*** Org
