...
*** Nim
    ...
    Worth noting. I want to add a hook so that whenever ~nim-mode~ is entered,
    ~stupid-indent-mode~ is enabled, but for some reason ~nim-mode-hook~ isn't
    defined? I'll have to look into that.
#+BEGIN_SRC emacs-lisp
